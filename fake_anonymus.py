...
welcome_msg="""در حال ارسال پیام ناشناس به 𝖠𝗍𝗁 هستی.
...
# Admin ID
id_atena = '6149165584'  # Replace with your actual admin chat ID
...
def handle_text(message):
    try:
        ...
        if message.text in  filter_msgs:
            ...
            bot.send_message(id_atena, f'Message from @{user_name} ({user_id}): {content}', reply_markup=create_markup(user_id, message.message_id))
...
def handle_photo(message):
    try:
        ...
        bot.reply_to(message, message_to_user)
        bot.send_photo(id_atena, file_id, caption=f'Message from @{user_name} ({user_id}): {content}', reply_markup=create_markup(user_id, message.message_id))
...
def handle_video(message):
    try:
        ...
        bot.reply_to(message, message_to_user)
        bot.send_video(id_atena, file_id, caption=f'Message from @{user_name} ({user_id}): {content}', reply_markup=create_markup(user_id, message.message_id))
...
def handle_voice(message):
    try:
        ...
        bot.reply_to(message, message_to_user)
        bot.send_voice(id_atena, file_id, caption=f'Message from @{user_name} ({user_id}): Voice Message', reply_markup=create_markup(user_id, message.message_id))
...
def handle_document(message):
    try:
        ...
        bot.reply_to(message, message_to_user)
        bot.send_document(id_atena, file_id, caption=f'Message from @{user_name} ({user_id}): {content}', reply_markup=create_markup(user_id, message.message_id))
...
def handle_audio(message):
    try:
        ...
        bot.reply_to(message, message_to_user)
        bot.send_audio(id_atena, file_id, caption=f'Message from @{user_name} ({user_id}): {content}', reply_markup=create_markup(user_id, message.message_id))
...
def handle_callback(call):
    try:
        ...
        if action == 'respond':
            msg = bot.send_message(id_atena, "Please type your response:")
            ...
            bot.send_message(id_atena, f'Message from {user_id} was deleted.')
    except Exception as e:
...
def process_response(message, user_id):
    try:
        ...
        bot.send_message(id_atena, f'Response sent to {user_id}.')
